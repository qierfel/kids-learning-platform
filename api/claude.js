...
[
  ...
  ...
    ...
    try {
      ...
      if (!response.ok) { const err = await response.text(); return res.status(502).json({ error: 'Upstream API error', detail: err }) }
      const data = await response.json()
      const text = data.content?.[0]?.text?.trim() || ''
      try {
        const jsonMatch = text.match(/\{[\s\S]*\}/)
        if (jsonMatch) { const parsed = JSON.parse(jsonMatch[0]); return res.status(200).json({ parsed }) }
      } catch { /* fall through */ }
      return res.status(200).json({ text })
