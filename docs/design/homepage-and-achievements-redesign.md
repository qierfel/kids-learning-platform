...
> 版本：v1.0 | 日期：2026-04-24 | 状态：待 Rita 确认
    ...
    items: [
      {
        id: "item_001",
        subject: "英语",             // 学科
        module: "SRS词汇",           // 模块名（对应路由/功能）
        moduleKey: "srs",            // 程序识别键
        targetValue: 20,             // 目标量
        targetUnit: "词",            // 单位
        completedValue: 0,           // 已完成量（由 activityLog 汇总）
        done: false
      },
      {
        id: "item_002",
...
| 问题 | 背景 | 建议选项 |
|------|------|----------|
| **学习时长统计口径** | 用户打开页面挂着不学怎么算？ | A. 仅统计有交互（点击/输入）的时段；B. 页面可见且无超 3 分钟空闲；C. 简单按 session 时长算（偏宽松）|
| **计划粒度** | 计划条目是"英语 SRS 20词"还是细到具体哪个 level？ | A. 模块级（推荐，简单）；B. 细到 level/难度 |
| **日计划 vs 周计划的关系** | 周计划是日计划的 7 个快照，还是独立存在？ | A. 完全独立（用户自己管）；B. 周计划按天均摊生成日计划（自动化） |
| **多设备/多浏览器** | activityLog 存 localStorage，换电脑数据丢失 | A. 接受，暂不同步（快速上线）；B. 同步到 KV（需要后端接口）|
| **家长视角** | 家长是否需要看孩子的学习状态？ | A. 暂不做；B. 做只读的家长视角（需要账号体系扩展）|
| **成就通知方式** | 解锁成就时如何告知用户？ | A. 页面内弹窗 Toast；B. 下次打开平台时显示；C. 不打扰，去成就页自己看 |
